...
def test_basic_tload(state_test: StateTestFiller, ):
    ...
    pre = {
        ...
        Account(
            ...
            + Op.TSTORE(16, 2) + Op.MSTORE(0, Op.GAS())  # hot load the memory
            + Op.MSTORE(0, Op.GAS()) + Op.TLOAD(16) + Op.MSTORE(32, Op.GAS()) +
            Op.SSTORE(tload_nonzero_gas_price_result,
                      Op.SUB(Op.MLOAD(0), Op.MLOAD(32))) +
            Op.SSTORE(tload_nonzero_gas_price_result,
                      Op.SUB(Op.SLOAD(16), extra_opcode_gas))
            # from zero slot
            + Op.MSTORE(0, Op.GAS()) + Op.TLOAD(5) + Op.MSTORE(32, Op.GAS()) +
